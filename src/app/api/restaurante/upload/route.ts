...
/** Upload de foto da loja ou do prato (área da loja) */
export async function POST(request: Request) {
  try {
    await exigirSessao("restaurante");
  } catch (e) {
    const mensagem =
      e instanceof Error ? e.message : "Faça login na área da loja.";
  ...
  const pastaRaw = String(form.get("pasta") ?? "lojas");
  const pasta = pastaRaw === "pratos" ? "lojas" : "lojas";
